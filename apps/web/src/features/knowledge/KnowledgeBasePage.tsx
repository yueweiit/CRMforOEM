...
function getKnowledgeList<T = unknown>(section: string) {
  const fetchers: Record<string, () => Promise<T>> = {
    brands: () => getBrands<T>(),
    products: () => getProducts<T>(),
    "oem-capabilities": () => getOemCapabilities<T>(),
    certificates: () => getCertificates<T>(),
    cases: () => getCaseStudies<T>(),
    "email-materials": () => getEmailMaterials<T>(),
  };
  return (fetchers[section] ?? (() => Promise.resolve([] as unknown as T)))();
}

function createKnowledgeItem<T = unknown>(section: string, payload: Record<string, unknown>, options?: { toast?: boolean }) {
  const creators: Record<string, (p: Record<string, unknown>, o?: { toast?: boolean }) => Promise<T>> = {
    brands: (p, o) => createBrand<T>(p, o),
    products: (p, o) => createProduct<T>(p, o),
    "oem-capabilities": (p, o) => createOemCapability<T>(p, o),
    certificates: (p, o) => createCertificate<T>(p, o),
    cases: (p, o) => createCaseStudy<T>(p, o),
    "email-materials": (p, o) => createEmailMaterial<T>(p, o),
  };
  return (creators[section] ?? (() => Promise.resolve(undefined as unknown as T)))(payload, options);
}

function updateKnowledgeItem<T = unknown>(section: string, id: string, payload: Record<string, unknown>, options?: { toast?: boolean }) {
  const updaters: Record<string, (id: string, p: Record<string, unknown>, o?: { toast?: boolean }) => Promise<T>> = {
    brands: (id, p, o) => updateBrand<T>(id, p, o),
    products: (id, p, o) => updateProduct<T>(id, p, o),
    "oem-capabilities": (id, p, o) => updateOemCapability<T>(id, p, o),
    certificates: (id, p, o) => updateCertificate<T>(id, p, o),
    cases: (id, p, o) => updateCaseStudy<T>(id, p, o),
    "email-materials": (id, p, o) => updateEmailMaterial<T>(id, p, o),
  };
  return (updaters[section] ?? (() => Promise.resolve(undefined as unknown as T)))(id, payload, options);
}

function deleteKnowledgeItem<T = unknown>(section: string, id: string, options?: { toast?: boolean }) {
  const deleters: Record<string, (id: string, o?: { toast?: boolean }) => Promise<T>> = {
    brands: (id, o) => deleteBrand<T>(id, o),
    products: (id, o) => deleteProduct<T>(id, o),
    "oem-capabilities": (id, o) => deleteOemCapability<T>(id, o),
    certificates: (id, o) => deleteCertificate<T>(id, o),
    cases: (id, o) => deleteCaseStudy<T>(id, o),
    "email-materials": (id, o) => deleteEmailMaterial<T>(id, o),
  };
  return (deleters[section] ?? (() => Promise.resolve(undefined as unknown as T)))(id, options);
}
...
export function KnowledgeBasePage() {
  ...
  const createMutation = useMutation({
    mutationFn: (payload: Record<string, unknown>) =>
      createKnowledgeItem<KnowledgeRecord>(currentSection, payload, { toast: false }),
    onSuccess: () => {
  ...
  const updateMutation = useMutation({
    mutationFn: (payload: Record<string, unknown>) =>
      updateKnowledgeItem<KnowledgeRecord>(currentSection, editingId, payload, { toast: false }),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => deleteKnowledgeItem(currentSection, id, { toast: false }),
    onSuccess: () => {
