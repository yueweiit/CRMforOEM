...
import { createSample, getSamples } from "../../../../api/customers";
...
export function SamplePanel({ customerId }: { customerId: string }) {
  ...
  const create = useMutation({ mutationFn: () => createSample({ ...form, customerId }), onSuccess: () => queryClient.invalidateQueries({ queryKey: ["samples", customerId] }) });
