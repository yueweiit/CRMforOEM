...
type CustomerBackgroundTaskView = {
  id: string;
...
function EmailPanel({ customer, customerId, onChanged }: { customer: CustomerDetail; customerId: string; onChanged: () => void }) {
  ...
  const generate = useMutation({
    mutationFn: () => apiPost<AcceptedResponse<{ id: string; status: string; message: string }>>(`/customers/${customerId}/email-drafts/generate`, cleanPayload(draftForm)),
    onSuccess: (response) => {
