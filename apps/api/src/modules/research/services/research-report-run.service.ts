...
export class ResearchReportRunService {
  ...
  async persistSuccess(params: {
    ...
    const { reportId, customerId, aiGenerationRunId, parsed, sourceEvidence, searchEnabled, completion } = params;
    ...
    const finalReport = await this.prisma.researchReport.update({
      ...
      data: {
        ...
        title: parsed.title, reportJson: parsed as never,
  ...
  async persistFailure(reportId: string, aiGenerationRunId: string | null, errorMessage: string) {
